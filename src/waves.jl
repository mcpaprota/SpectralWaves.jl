function linear_regular_wave!(p::Problem, H, L, nΔt)
    ...
    p.η̂[ℐ, O] = a / 2
    p.η̂[ℐ+2, O] = a / 2
    [p.η̇[ℐ, i] = im * ω * a / 2 * exp((i - O) * im * ω * Δt) for i in 1:O]
    [p.η̇[ℐ+2, i] = -im * ω * a / 2 * exp(-(i - O) * im * ω * Δt) for i in 1:O]
    p.ϕ̂[ℐ, O] = im * a * g / ω / 2
    p.ϕ̂[ℐ+2, O] = -im * a * g / ω / 2
    [p.ϕ̇[ℐ, i] = -g * a / 2 * exp((i - O) * im * ω * Δt) for i in 1:O]
    [p.ϕ̇[ℐ+2, i] = -g * a / 2 * exp(-(i - O) * im * ω * Δt) for i in 1:O]
    return T, Δt
end
